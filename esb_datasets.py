...
class ESBDatasets(datasets.GeneratorBasedBuilder):
    ...
    def _librispeech_split_generators(self, dl_manager):
        ...
        requested_train_splits = os.getenv("ESB_LIBRISPEECH_TRAIN_SPLITS")
        if requested_train_splits:
            aliases = {
                "clean.100": "train.clean.100",
                "clean.360": "train.clean.360",
                "other.500": "train.other.500",
            }
            train_splits = [
                aliases.get(split.strip(), split.strip())
                for split in requested_train_splits.split(",")
                if split.strip()
            ]
        dl_urls = {config_name: _LIBRISPEECH_DL_URLS[config_name] for config_name in train_splits + dev_splits + test_splits}
        local_archive_dir = os.getenv("ESB_LOCAL_ARCHIVE_DIR")
        if local_archive_dir:
            for config_name in list(dl_urls):
                local_path = os.path.join(local_archive_dir, os.path.basename(dl_urls[config_name]))
                if os.path.exists(local_path):
                    dl_urls[config_name] = local_path
...
_LIBRISPEECH_DL_URL = "https://www.openslr.org/resources/12/"
